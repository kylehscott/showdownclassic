...
var awayTeam = {
  ...
  currentIP: 0,
  currentER: 0,
  bench: awayTeamData.lineup.slice(8)
}
...
var changeBatter = function(batter) {
  if (batter.pos == "Starter" || batter.pos == "Reliever" || batter.pos == "Closer") {
    $(".batters-img").attr("src", playerImage(batter));
    $(".batter-name").text(htmlDecode(batter.name));
    $("#batter-year").text("'" + batter.cardYear.substring(2));
    $("#batter-team").text(batter.team);
    $(".batter-ob").text("OB: -");
    $(".batter-speed").text("-");
    $("#batter-so").text("-");
    $("#batter-gb").text("-");
    ...
    $("#batter-3b").text("-");
    $("#batter-hr").text("-");
    $("#batter-pos").text(" - " + batter.pos);
  } else {
    $(".batters-img").attr("src", playerImage(batter));
    $(".batter-name").text(htmlDecode(batter.name));
    $("#batter-year").text("'" + batter.cardYear.substring(2));
    $("#batter-team").text(batter.team);
    $(".batter-ob").text(batter.onBase);
    $(".batter-speed").text(batter.speed);
    statsData = [batter.strikeOut, batter.groundBall, batter.flyBall, batter.walk, batter.single, batter.singlePlus, batter.double, batter.triple, batter.homeRun];
    statsAbbr = ["SO", "GB", "FB", "BB", "1B", "1B+", "2B", "3B", "HR"];
    statsStr = ""
    for (i = 0; i < statsData.length; i++) {
      if (statsData[i] != "-") {
        if (statsData[i].split("-")[1] == "") {
          statsData[i] = statsData[i].split("-")[0] //statsData[i].split("-")
        }
        statsStr = statsStr + statsAbbr[i] + ": " + statsData[i] + "<br>";
      }
    }
    $(".stats-div").html(statsStr);
    $("#batter-pos").text(" - " + $("#" + currentTeam.text + "-batter-" + (currentTeam.batter + 1) + "-pos").text());
  }
}
...
var changePitcher = function(pitcher) {
  if (pitcher.pos == "Starter" || pitcher.pos == "Reliever" || pitcher.pos == "Closer") {
    ...
    $("#pitcher-year").text("'" + pitcher.cardYear.substring(2));
    $("#pitcher-team").text(pitcher.team);
    $(".pitcher-con").text(pitcher.onBase);
...
var checkResult = function (player, roll) {
  if (roll == player.popUp.split('-')[0] || roll <= player.popUp.split('-')[1]) {
    ...
    outs += 1;
    if (outs < 3) {
      if (basePosition[2] != null && basePosition[2].speed == "B") {
        basePosition.push(basePosition[2]);
        basePosition[2] = null;
      }
      if (basePosition[1] != null && basePosition[2] == null && basePosition[1].speed == "A") {
...
var updateBases = function() {
  ...
  for (i = 0; i < 3; i++) {
    if (basePosition[i] != null){
      $(bases[i] + "-blank").attr("hidden", true);
      $(bases[i] + "-div").prop("hidden", false);
      ...
      if (basePosition[i].pos == "Starter" || basePosition[i].pos == "Reliever" || basePosition[i].pos == "Closer") {
        basePosition[i].speed = "C";
      }
      $(bases[i] + "-speed").text(basePosition[i].speed);
      ...
      $(bases[i] + "-div").attr("hidden", true);
      $(bases[i] + "-blank").attr("hidden", false);
    }
  };
...
UIkit.util.on('#js-modal-dialog', 'click', function (e) {
    $(".model-dialog").addClass("uk-margin-auto-vertical");
    e.preventDefault();
    e.target.blur();

    statsAbbr = ["SO", "GB", "FB", "BB", "1B", "1B+", "2B", "3B", "HR"];
    var homeBench = "";
    for (i = 0; i < awayTeam.bench.length; i++) {
        console.log(i);
        var player = awayTeam.bench[i];

        var statsData = [player.strikeOut, player.groundBall, player.flyBall, player.walk, player.single, player.singlePlus, player.double, player.triple, player.homeRun];
        var playerStr = '<div class="uk-width"><div class="flip-container" ontouchstart="this.classList.toggle("hover");"> \
                <div class="flipper" id="batter-flipper"> \
                    <div id="batter" class="front uk-card uk-background-secondary" style="font-size: 13px;"> \
                        <img src="/static/image/card_imgs/' + player.cardYear + '/' + player.cardSet + '/' + player.cardNo + '.jpg"> \
                        <div class="uk-overlay uk-position-top-right uk-padding-remove-top uk-padding-remove-right uk-text-right"> \
                            <small class="uk-label uk-background-secondary" style="font-size: 10px"><span class="uk-visible@s">OB: </span><span">' + player.onBase + '</span></small><br> \
                            <small class="uk-label uk-background-secondary uk-visible@m" style="font-size: 8px;"><span class="uk-visible@m">SPD: ' + player.speed + '</span></small> \
                        </div> \
                        <div class="card-overlay uk-overlay uk-overlay-primary uk-position-bottom uk-padding-small"> \
                            <div class="uk-flex-nowrap"><span class="batter-name">' + player.name + '</span><br></div> \
                            <span class="uk-visible@m" style="font-size: 10px; font-style: italic;"><span id="batter-year">' + player.cardYear + '</span> <span id="batter-team">' + player.team + '</span></span> \
                        </div> \
                    </div> \
        ...
        homeBench = homeBench + (playerStr);

    }



    UIkit.modal.dialog('<div class="uk-flex uk-flex-row uk-flex-center uk-modal-body uk-background-secondary"><br>' + homeBench + '</div');
});
